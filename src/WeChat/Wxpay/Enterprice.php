...
class WxPayConf_pub
{
	...
	{
		if (! $options) die("未配置支付信息.");

		static::$appid = $options['appid'];
		static::$mchid = $options['mchid'];
		static::$key   = $options['key'];
		static::$appsecret  = $options['appsecret'];
	}
}
...
class Common_util_pub
{
	 ...
	 */
	public function postXmlSSLCurl($xml,$url,$second=30, $cert_path = '')
	{
		$ch = curl_init();
		//超时时间
		...
		//curl_setopt($ch,CURLOPT_CAINFO, $current_path . '/config/cacert/rootca.pem');
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub
{
	 ...
	 * 生成企业付款接口参数xml
	 */
	function createXml_qiye($mch_billno = '')
	{
		try {
			$this->parameters["wxappid"] = WxPayConf_pub::$appid; //公众账号ID
			$this->parameters["mch_id"] = WxPayConf_pub::$mchid;  //商户号
			$this->parameters["mch_billno"] = $mch_billno ?: WxPayConf_pub::$mchid . date('YmdHis') . mt_rand(1000,9999); // 商户订单号
